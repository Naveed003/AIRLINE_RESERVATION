...
def NEW_BOOKING():
    ...
    def date_input():  # taking input for departure date and checking validity
        ...
        while True:
            ...
            year=int(depature_date[0:4])
            mont=int(depature_date[5:7])
            dayyyy=int(depature_date[-2:])
            print(year,mont,dayyyy)
            temp_date=datetime(year,mont,dayyyy)
            listtttt=[i for i in range(10,365,10)]
            temp_date-=timedelta(days=random.choice(listtttt))
            ...
            if len(depature_date) != 10:
                ...
                print("\n", "="*4, 'Please Enter a Valid Date ', "="*4)
            elif str(current_date) <= depature_date or str(current_date) >= depature_date: #and str(f_date) > depature_date:
        ...
        def f_confirmation():
            ...
            try:
                ...
                if res != []:
                    ...
                    query = "INSERT INTO CUSTOMERS VALUES({},'{}','{}','{}','{}','{}','{}','{}','{}')".format(
                        details[0], details[2], details[3], details[4], details[5], details[6], details[-2], details[-3], temp_date)
                    mycursor.execute(query)
                for i in range(len(sel)):
                    query = 'INSERT INTO BOOKINGS VALUES("{}","{}","{}","{}","{}","{}","{}","{}")'.format(
                        details[0], details[1], sel.loc[i, 'FLIGHT NO'], sel.loc[i, 'DEPARTURE_TIME'], list_seat[i], list_seat_id[i], sel.loc[i, 'PRICE (USD)'], temp_date)
                    mycursor.execute(query)
